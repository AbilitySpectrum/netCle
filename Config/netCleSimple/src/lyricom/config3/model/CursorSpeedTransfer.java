 ...
 * @author Andrew
 */
public class CursorSpeedTransfer {
    private static CursorSpeedTransfer instance = null;
    
    public static CursorSpeedTransfer getInstance() {
        if (instance == null) {
            instance = new CursorSpeedTransfer();
        }
        return instance;
    }
    
    private Converter converter = new Converter();
    private CursorSpeedTransferInterface transfer;
    
    private CursorSpeedTransfer() {
        
    }
    
    // Called by the UI component to register its location.
    public void registerUIComponent(CursorSpeedTransferInterface msti) {
        transfer = msti;
    }
    
    public void toStream(OutStream os) {
        int[] values;
        if (transfer == null) return; // No mouse speed changes.
