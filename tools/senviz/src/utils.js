...
const SENSACT_PROTOCOL = "1.1"
const EXP2_PROTOCOL = "2.0"
var PROTOCOL = SENSACT_PROTOCOL // EXP2_PROTOCOL
...
function decode_output(output_bytes, parameter_bytes) {
    ...
    if (output === 10) { out.action = 'Set State'; out.outid = parameters >> 8; out.sensor = INPUTS[parameters >> 8]; out.sensor_state = parameters & 0xff; return out }
...
function translate_trigger(bytes) {
    ...
    trigger.id = decode_id(bytes.substring(0, 2));
...
function decode_triggers_2(bytes, trigger_length) {
    let count = 0
    let triggers = [], trigger
    let current_input = null
    let sensor_triggers = []
    while ((bytes.length > 0) && (bytes[0] !== 'Y')) {
        trigger = bytes.substring(0, trigger_length)

        trigger = translate_trigger(trigger)
        //updateOutput(JSON.stringify(trigger))

        if (current_input != trigger["input"]) {
            if (current_input !== null) {
                triggers.push({ "sensor": current_input, "triggers": sensor_triggers })
            }
            current_input = trigger["input"]
            sensor_triggers = []
        }
        sensor_triggers.push(trigger)
        //updateOutput(JSON.stringify(triggers[current_input].states))
...
async function parse_configuration(bytes) {
    // parese into global Triggers and Mouse_Paramenters
    ...
    Triggers = reformat_triggers(triggers)
    //updateOutput( prettyPrintJson.toHtml(JSON.stringify(Triggers)))
    ...
    if (bytes.indexOf('Y') >= 0) {
        ...
        Mouse_Parameters = decode_mouse_parameters(bytes.slice(bytes.indexOf('Y')))
    ...
    try {
        ...
        */
        ...
        while (true) {
            ...
            if (PROTOCOL == EXP2_PROTOCOL) {
                // code to read 9250 data -- Arduino code: MPU9250_exp-s2.ino
                buf8 = concatTypedArrays(buf8, value)

                // Read data from serial port if available and do a header check
                while (buf8.length >= 14) {
                    // console.log("+"+buf.length+" "+buf[0] + " "+buf[1])
                    // console.log("-"+buf.slice(14).length+" "+buf.slice(14)[0] + " "+buf.slice(14)[1])
                    if ((buf8[0] == 0x9f) && (buf8[1] == 0x6e)) {
                        // Cast the bytes into usable values
                        sensor_value[8] = bytes2num(buf8[3], buf8[2]);
                        if (sensor_value[8] > 180) sensor_value[8] = sensor_value[8] - 360
                        meters[8].value = sensor_value[8]

                        meters[9].value = sensor_value[9] = bytes2num(buf8[5], buf8[4]);
                        meters[10].value = sensor_value[10] = bytes2num(buf8[7], buf8[6]);
                        meters[11].value = sensor_value[11] = bytes2num(buf8[9], buf8[8]);
                        meters[12].value = sensor_value[12] = bytes2num(buf8[11], buf8[10]);
                        meters[13].value = sensor_value[13] = bytes2num(buf8[13], buf8[12]);

                        dataLastUpdated = millis();

                        buf8 = buf8.slice(14)

                        // console.log(sensor_value[8] + " " + sensor_value[9] + " " + sensor_value[10] + " / " + sensor_value[11] + " " + sensor_value[12] + " " + sensor_value[13])
                    }
                    else {
                        //console.log('-1')
                        buf8 = buf8.slice(1)
            ...
            else if (PROTOCOL == SENSACT_PROTOCOL) {
                //below is sensact 1.1 protocol
                buffer = buffer + decoder.decode(value).replace(/[^0-9\'-zA-Z]/g, '')

                // console.log("Buffer:", buffer, "\n");

                // Look for end of response
                while (buffer.indexOf('Z') > 0) {   //ylh: this change (from if to while) is important: we need to consume buffer as quickly as possible
                ...
                alert("Unimplemented comms protocol " + PROTOCOL)
            }
        }
