/* MPU9250_MS5637_t3 Basic Example Code
  ...
  ylh : 2020-05-08 Fri : attempt to modify this from Teensy-only
        2021-04-28 Wed : different output formats

  https://github.com/kriswiner/MPU9250

  (A) When #define OUTPUT_FOR_VISUALIZER 
  then outputs 14byte format and can be visualize using
  the visualization software comes from https://github.com/MarkSherstan/MPU-6050-9250-I2C-CompFilter

  (B) Otherwise,
  outputs in Sensact 1.1 protocol pretending to by Accel-X,Y,Z
...
//#include "Wire.h"
// ylh #include <i2c_t3.h>

//#define OUTPUT_FOR_VISUALIZER

#include "I2Cdev.h"

#include <SPI.h>

// bring in needed netClé stuff
#include "netCle.h"
#include "Sensors.h"
#include "IO.h"
#include <Mouse.h>
long lastActionTime = 0, lastReportTime = 0;
int gYaw = 0, gPitch = 0, gRoll = 0; //?? global for now
extern Sensors sensors;
SerialOutputStream serialOutput;


/* ylh - get rid of all display
...
void setup()
{
  ...
  digitalWrite(intPin, LOW); // ylh added
  ...
  if (c == 0x71) // WHO_AM_I should always be 0x68
  {
    ...
    // ylh - calibration to skip
    //     magcalMPU9250(magBias, magScale);

    /*ylh - precalibrated - first 9250
      magBias[0] = 412.92;
      magBias[1] = 679.22;
      magBias[2] = 261.57;
...
void loop()
{
  ...
  if ( cmd == 'U' || cmd == 'D' || cmd == 'L' || cmd == 'R') {
    ...
    if (cmd == 'U') {
      Mouse.move(0, -10);
      //      Serial.print("W"); Serial.print(cmd); Serial.println("Z");
    }
    else if ( cmd == 'D') {
      Mouse.move(0, 10);
      //      Serial.print("W"); Serial.print(cmd); Serial.println("Z");
...
#ifdef OUTPUT_FOR_VISUALIZER
  // we write to output in MPU9250loop()
#else
  const SensorData *pSensorData;
  if ((lastReportTime + REPORTING_INTERVAL) < millis()) {
...
#else
void MPU9250::init() {};
void MPU9250::getValues(SensorData *pData) {
  pData->addValue(Yaw, gYaw);  //??
  pData->addValue(Pitch, gPitch);
  pData->addValue(Roll, gRoll);
...
void MPU9250loop() {
  ...
  //  if (newData == true) { // On interrupt, read data  // ylh - use the following instead
  ...
  if (delt_t > 100) { // update LCD once per half-second independent of read rate
    ...
    if (SerialDebug) {
...
#ifdef OUTPUT_FOR_VISUALIZER
      // for the Visualizer : ylh : https://github.com/MarkSherstan/MPU-6050-9250-I2C-CompFilter
      // Cast to a byte pointer(s)
      int y = (int) yaw;
      int p = (int) pitch;
      int r = (int) roll;

      byte* byteData1 = (byte*)(&y);
      byte* byteData2 = (byte*)(&p);
      byte* byteData3 = (byte*)(&r);
      byte* byteData4 = (byte*)(0);
      byte* byteData5 = (byte*)(0);
      byte* byteData6 = (byte*)(0);
      ...
      Serial.write(buf, 14);
#else
      // sensact 1.1 protocol
      gYaw = (int) (yaw * 10.0f); // in degrees*10 units
      gPitch = (int) (pitch * 10.0f);
      gRoll = (int) (roll * 10.0f);

#endif

      ahrsind = !ahrsind;
      digitalWrite(LED_GREEN, ahrsind);
...
void MS5637PromRead(uint16_t * destination)
{
  ...
  for (uint8_t ii = 0; ii < 7; ii++) {
    ...
    Wire.endTransmission(false); // ylh
    uint8_t i = 0;
...
uint32_t MS5637Read(uint8_t CMD, uint8_t OSR)  // temperature data read
{
  ...
  //  ylh - Wire.endTransmission(I2C_NOSTOP);        // Send the Tx buffer, but send a restart to keep connection alive
